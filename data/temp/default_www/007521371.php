                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody> <?php if (empty($list)):?> <tr>
                                    ...
                                    </tr> <?php endif;?><?php unset($k, $v); $k_index = 0; foreach ((array)$list as $k => $v): $k_index ++;?> <tr id="item-<?php echo($v['group_id']);?>">
                                        <td>
                                            <input type="checkbox" class="i-checks" value="<?php echo($v['group_id']);?>" />
                                      </td>

                                        <td><?php echo($v['group_label']);?></td>
                                        <td class="group-perms"> <?php if (!empty($v['group_perm'])):?> <?php unset($sk, $sv); $sk_index = 0; foreach ((array)$v['group_perm'] as $sk => $sv): $sk_index ++;?> <span><?php echo($perms[$sv]);?></span> <?php endforeach;?> <?php else:?> 所有操作 <?php endif;?></td>
                                        <td>
                                            <a class="btn btn-xs btn-success" title="编辑<?php echo($v['group_label']);?>" href="<?php echo(RGX\router::url('%s-add-id-%d' , $_MODULE, $v['group_id'])); ?>">编辑</a> <?php if ($v['group_id'] == 1):?> <a class="btn btn-xs btn-danger disabled">删除</a> <?php else:?> <a href="<?php echo(RGX\router::url('%s-del-id-%d' , $_MODULE, $v['group_id'])); ?>" data-confirm="操作不可撤销，确认吗？" data-target="#item-<?php echo($v['group_id']);?>" class="btn btn-xs btn-danger btn-delete">
                                                删除
                                            </a> <?php endif;?></td>
